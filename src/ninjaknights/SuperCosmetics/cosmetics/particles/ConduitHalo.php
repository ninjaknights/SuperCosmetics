...
use ninjaknights\SuperCosmetics\cosmetics\particles\generic\ConduitParticle;
    ...
    public function onRun(): void
    {
        foreach ($this->plugin->getServer()->getOnlinePlayers() as $player) {
            if ($this->plugin->hasEnabledParticle($player)) {
                ...
                $player->getWorld()->addParticle(new Vector3($x - $b, $y + $c + $d + 1.2, $z - $a), new ConduitParticle());
                $player->getWorld()->addParticle(new Vector3($x + $a, $y + $c + $d + 1.2, $z + $b), new ConduitParticle());
                $player->getWorld()->addParticle(new Vector3($x + $b, $y + $c + $d + 1.2, $z - $a), new ConduitParticle());
                $player->getWorld()->addParticle(new Vector3($x + $a, $y + $c + $d + 1.2, $z - $b), new ConduitParticle());
                $player->getWorld()->addParticle(new Vector3($x + $a, $y + 2, $z + $b), new ConduitParticle());
                $this->count++;
            }
        }
    }
