<?php
/*
 * Thanks to AngelliaX Clothes plugin we were able to make hats, suits and morphs a thing.
 * So go check out the plugin and support the creator
 * The Plugins Github: https://github.com/AngelliaX/Clothes
 * Creator Github: https://github.com/AngelliaX
 */
namespace ninjaknights\SuperCosmetics\util\skin;
...
class SkinUtil
{
    ...
    //Adds the skin on top the player || Combines the player skin and the skin that is set || Example: Hats
    ...
    public function getSkins()
    {
        ...
        foreach ($allDirs as $foldersName) {
            if (is_dir($path . $foldersName)) {
                ...
                $main->skinNames[$foldersName] = $checkFileAvailable;
                sort($main->skinNames[$foldersName]);
                $checkFileAvailable = [];
            }
        }
        unset($main->skinTypes[0]);
        unset($main->skinTypes[1]);
        unset($main->skinTypes[array_search("saveskin", $main->skinTypes)]);
        unset($main->skinNames["."]);
        unset($main->skinNames[".."]);
        unset($main->skinNames["saveskin"]);
        sort($main->skinTypes);
    }
}